...
phrasebook, theming) ships alongside voice, not instead of it — voice is the
keyboard's default landing mode. Both panels share a hand-drawn vector icon set
and a from-scratch synthesized sound-effect system (key taps, Enter, and
recording start/accept/failure chimes, toggleable in Settings), plus an in-app
...
The source is public on GitHub ([shubhanksargam/Private-Voice](https://github.com/shubhanksargam/Private-Voice)),
but no Play Store listing exists yet — release signing, version metadata, and a model
distribution path (the app has no `INTERNET` permission, so there's no in-app download
option) are all still open. See [`docs/RELEASE_CHECKLIST.md`](docs/RELEASE_CHECKLIST.md)
...
```
android/
  app/      IME, keyboard UI (typing + voice), vector icon set, synthesized
            sound effects, RecognitionService, settings, phrasebook, guide
            screen, M0 benchmark harness
...
This project's own code is all rights reserved — public for viewing on GitHub, but
no licence is granted to use, copy, modify, or redistribute it. See [`LICENSE`](LICENSE).

© 2026 Shubhank Sargam.
